'use client';
import Image from 'next/image';import Link from 'next/link';import {Check,Heart,Play,ShoppingBag,Clock,Ban} from 'lucide-react';import {useState} from 'react';import {Product,formatNaira} from '@/lib/products';import {useCart} from './CartProvider';import {useWishlist} from './WishlistProvider';import {useRequireAuth} from './useRequireAuth';
export default function ProductCard({product,hoverVideo}:{product:Product;hoverVideo?:boolean}){
  const [added,setAdded]=useState(false);const [hovering,setHovering]=useState(false);const {add}=useCart();const {toggle,has}=useWishlist();const liked=has(product.id);const {requireAuth}=useRequireAuth();
  ...
  const badge = product.oldPrice ? {label:'Sale',className:'bg-[#F50063] text-white'}
    ...
    : null;
  const showVideo = hoverVideo && !!product.video && hovering;
  function handleAdd(){ if(outOfStock) return; requireAuth(()=>{add(product.id);setAdded(true);window.setTimeout(()=>setAdded(false),1400)}) }
  function handleWishlist(){ requireAuth(()=>toggle(product.id)) }
  return (
    <article className="group flex w-full min-w-0 flex-col" onMouseEnter={()=>setHovering(true)} onMouseLeave={()=>setHovering(false)}>
      <div className="relative aspect-[4/5] w-full min-w-0 overflow-hidden rounded-2xl bg-[#ecebe6] shadow-[0_18px_50px_rgba(0,0,0,.04)]">
        <Link href={`/product/${product.slug}`} aria-label={`View ${product.name}`} className="absolute inset-0 block">
          {showVideo ? (
            <video src={product.video} autoPlay muted loop playsInline className="absolute inset-0 h-full w-full object-cover" />
          ) : (
        ...
        {product.video&&!showVideo&&<span className="pointer-events-none absolute bottom-3 right-3 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[rgba(255,255,255,0.92)] shadow-sm backdrop-blur"><Play size={12} fill="currentColor"/></span>}
      </div>
