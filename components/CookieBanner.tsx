...
export default function CookieBanner() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-[90] p-3 sm:p-4">
      <div className="mx-auto max-w-lg rounded-2xl border border-[rgba(0,0,0,0.1)] bg-white p-4 shadow-[0_18px_50px_rgba(6,20,38,.14)] sm:p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-xs leading-5 text-[rgba(0,0,0,0.55)]">We use essential cookies to keep Vendly working properly. We may also use optional cookies to understand how customers use our store and improve your experience.</p>
            {managing && (
              <div className="mt-3 rounded-xl bg-[#F7F8FA] p-3 text-xs text-[rgba(0,0,0,0.6)]">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {!managing && <button onClick={() => setManaging(true)} className="px-2! py-2! text-xs font-bold text-[rgba(0,0,0,0.5)] underline underline-offset-4">Manage preferences</button>}
            </div>
          </div>
          <button onClick={() => choose('essential')} aria-label="Dismiss" className="text-[rgba(0,0,0,0.3)] hover:text-[rgba(0,0,0,0.6)]"><X size={16} /></button>
        </div>
      </div>
    </div>
  );
