...
export default function AdminShell({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen bg-[#F7F8FA] text-[#161616]">
    <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#061426] transition-transform lg:translate-x-0 ${open?'translate-x-0':'-translate-x-full'}`}>
      <div className="flex h-full flex-col p-5">
        <div className="flex items-center justify-between px-2">
          ...
          <button onClick={()=>setOpen(false)} className="rounded-lg p-2 text-white/70 hover:bg-white/10 lg:hidden"><X size={19}/></button>
        </div>
        <div className="mt-8 rounded-2xl bg-white/[.06] p-4 text-white">
          <p className="text-[10px] font-bold uppercase tracking-[.18em] text-white/40">Control centre</p>
          <p className="mt-2 text-sm font-semibold">Store operations</p>
          <p className="mt-1 text-xs text-white/40">Catalogue, orders &amp; customers</p>
        </div>
        <nav className="mt-7 space-y-1">{nav.map(([label,href,Icon])=>{const active=path===href||path.startsWith(href+'/');return <Link key={href} href={href} onClick={()=>setOpen(false)} className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active?'bg-white/10 text-white':'text-white/55 hover:bg-white/[.06] hover:text-white'}`}>{active&&<span className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-r-full bg-[#FF7200]"/>}<Icon size={18}/>{label}</Link>})}</nav>
        <div className="mt-auto border-t border-white/10 pt-4">
          <Link href="/" className="mb-2 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-white/55 hover:bg-white/[.06] hover:text-white"><Store size={18}/>View storefront</Link>
          <LogoutButton/>
        </div>
      </div>
    </aside>
